...
var EndpointWhere = struct {
	...
	ID:        whereHelperint{field: "\"endpoint\".\"id\""},
	Data:      whereHelpernull_String{field: "\"endpoint\".\"data\""},
	ProjectID: whereHelperint{field: "\"endpoint\".\"project_id\""},
}
...
var (
	endpointAllColumns            = []string{"id", "data", "project_id"}
	endpointColumnsWithoutDefault = []string{"data", "project_id"}
	endpointColumnsWithDefault    = []string{"id"}
	endpointPrimaryKeyColumns     = []string{"id"}
)

type (
	// EndpointSlice is an alias for a slice of pointers to Endpoint.
	// This should generally be used opposed to []Endpoint.
...
func (q endpointQuery) One(ctx context.Context, exec boil.ContextExecutor) (*Endpoint, error) {
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "models: failed to execute a one query for endpoint")
	}
...
func (q endpointQuery) Count(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to count endpoint rows")
	}
...
func (q endpointQuery) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	...
	err := q.Query.QueryRowContext(ctx, exec).Scan(&count)
	if err != nil {
		return false, errors.Wrap(err, "models: failed to check if endpoint exists")
	}
...
func Endpoints(mods ...qm.QueryMod) endpointQuery {
	mods = append(mods, qm.From("\"endpoint\""))
...
func FindEndpoint(ctx context.Context, exec boil.ContextExecutor, iD int, selectCols ...string) (*Endpoint, error) {
	...
	query := fmt.Sprintf(
		"select %s from \"endpoint\" where \"id\"=$1", sel,
	)
	...
	if err != nil {
		...
		return nil, errors.Wrap(err, "models: unable to select from endpoint")
	}
...
func (o *Endpoint) Insert(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) error {
	if o == nil {
		return errors.New("models: no endpoint provided for insertion")
	}
	...
	if !cached {
		...
		if len(wl) != 0 {
			cache.query = fmt.Sprintf("INSERT INTO \"endpoint\" (\"%s\") %%sVALUES (%s)%%s", strings.Join(wl, "\",\""), strmangle.Placeholders(dialect.UseIndexPlaceholders, len(wl), 1, 1))
		} else {
			cache.query = "INSERT INTO \"endpoint\" %sDEFAULT VALUES%s"
		}
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to insert into endpoint")
	}
...
func (o *Endpoint) Update(ctx context.Context, exec boil.ContextExecutor, columns boil.Columns) (int64, error) {
	...
	if !cached {
		...
		if len(wl) == 0 {
			return 0, errors.New("models: unable to update endpoint, could not build whitelist")
		}

		cache.query = fmt.Sprintf("UPDATE \"endpoint\" SET %s WHERE %s",
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to update endpoint row")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by update for endpoint")
	}
...
func (q endpointQuery) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to update all for endpoint")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to retrieve rows affected for endpoint")
	}
...
func (o EndpointSlice) UpdateAll(ctx context.Context, exec boil.ContextExecutor, cols M) (int64, error) {
	...
	sql := fmt.Sprintf("UPDATE \"endpoint\" SET %s WHERE %s",
...
func (o *Endpoint) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns) error {
	if o == nil {
		return errors.New("models: no endpoint provided for upsert")
	}
	...
	if !cached {
		...
		if updateOnConflict && len(update) == 0 {
			return errors.New("models: unable to upsert endpoint, could not build update column list")
		}
		...
		cache.query = buildUpsertQueryPostgres(dialect, "\"endpoint\"", updateOnConflict, ret, update, conflict, insert)
	...
	if err != nil {
		return errors.Wrap(err, "models: unable to upsert endpoint")
	}
...
func (o *Endpoint) Delete(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	result, err := exec.ExecContext(ctx, sql, args...)
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to delete from endpoint")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by delete for endpoint")
	}
...
func (q endpointQuery) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	result, err := q.Query.ExecContext(ctx, exec)
	if err != nil {
		return 0, errors.Wrap(err, "models: unable to delete all from endpoint")
	}

	rowsAff, err := result.RowsAffected()
	if err != nil {
		return 0, errors.Wrap(err, "models: failed to get rows affected by deleteall for endpoint")
	}
...
func (o EndpointSlice) DeleteAll(ctx context.Context, exec boil.ContextExecutor) (int64, error) {
	...
	sql := "DELETE FROM \"endpoint\" WHERE " +
...
func (o *EndpointSlice) ReloadAll(ctx context.Context, exec boil.ContextExecutor) error {
	...
	sql := "SELECT \"endpoint\".* FROM \"endpoint\" WHERE " +
		strmangle.WhereClauseRepeated(string(dialect.LQ), string(dialect.RQ), 1, endpointPrimaryKeyColumns, len(*o))
...
func EndpointExists(ctx context.Context, exec boil.ContextExecutor, iD int) (bool, error) {
	var exists bool
	sql := "select exists(select 1 from \"endpoint\" where \"id\"=$1 limit 1)"
	...
	err := row.Scan(&exists)
	if err != nil {
		return false, errors.Wrap(err, "models: unable to check if endpoint exists")
	}
